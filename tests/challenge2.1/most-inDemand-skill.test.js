...
const j3 = new Job('job3', '', [new Skill('S3', 1)], undefined, new Date(2023, 2, 1));
const j4 = new Job('job3', '', [new Skill('S1')], undefined, new Date(2023, 3, 1));

test('Most in demand skill test', () => {
  expect(Utils.mostInDemandSkill([]))
    .toEqual([]);
});
...
test('Most in demand skill test', () => {
  expect(Utils.mostInDemandSkill([j2, j3]))
    .toEqual(['S3']);
});

test('Most in demand skill test', () => {
  expect(Utils.mostInDemandSkill([j1, j3]))
    .toEqual(['S3']);
});
